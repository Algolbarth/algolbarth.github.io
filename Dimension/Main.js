function demarrage() {
    ...
    Compte.music.setAttribute('loop', 'loop');
...
function ecran_titre() {
    ...
    initialiser();
    fonction("<div class='ecran_titre'>Cliquez pour démarrer</div>", 'document.body.style.backgroundImage="none";Compte.music.play();menu()');
    actualiser();
}
...
function connexion() {
    ...
    saut(2);
    afficher("Charger une sauvegarde : <input type='file' onchange='charger_save(this.files[0])'>");
    saut(2);
...
async function charger_save(file) {
...
function rules() {
    ...
    afficher("<div style='color:red'>ATTENTION : Pour sauvegarder votre partie, allez dans l'onglet qui porte le nom de votre compte, puis récupérer le code de sauvegarde. Ce code vous permettra de récupérer votre compte dans l'état, ne le perdez pas.</div>");
    afficher("</center>");
    actualiser();
}
...
function compte() {
    ...
    saut();
    afficher(code_sauvegarde());
    saut(2);
    fonction("<div class='bouton'>Télécharger la sauvegarde</div>", "sauvegarde()");
    afficher("</center>");
    actualiser();
}

function sauvegarde () {
    telecharger(Compte.nom, code_sauvegarde());
}

function code_sauvegarde () {
