...
function banque_deposer (objet_slot,nombre,pnj_id,dialogue_id) {
	banque_ajouter(Jeu.inventaire[objet_slot].id,nombre);
...
function banque_retirer (objet_id,nombre,pnj_id,dialogue_id) {
	objet_ajouter(Jeu.banque.inventaire[objet_id].id,nombre);
	banque_enlever(objet_id,nombre);
	banque_choisir_retirer(pnj_id,dialogue_id);
}

function banque_ajouter (objet_id,nombre) {
	let verifier = banque_verifier(objet_id);
    if (verifier !== false) {
        Jeu.banque.inventaire[verifier].nombre += nombre;
    }
    else {
        Jeu.banque.inventaire.push(obtenir_objet(objet_id,nombre));
    }
}
...
function banque_verifier (objet_id) {
    for (let n=0;n<Jeu.banque.inventaire.length;n++) {
        if (Jeu.banque.inventaire[n].id == objet_id) {
            return n;
        }
    }
    return false;
